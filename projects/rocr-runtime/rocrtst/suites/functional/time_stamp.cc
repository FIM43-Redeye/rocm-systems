...
void TimeStamp:: FreeSignals(void){
  // deallocating the signals
...
void  TimeStamp:: FreeQueue(void){
   //deallocating the Queue.
    if (queue)
      hsa_queue_destroy(queue);
}

void  TimeStamp:: FreeResources(void){
  FreeSignals();
  FreeQueue();
}
void TimeStamp::BarrierPacketTimestampValidationTest(void) {
  ...
  for (uint32_t i = 0; i < NUM_BARRIERS; ++i) {
    ...
    if(dispatch_times[i].start > dispatch_times[i].end){
      FreeResources();
      ASSERT_GE(dispatch_times[i].end, dispatch_times[i].start);
    }
    if(i > 0 && dispatch_times[i - 1].end > dispatch_times[i].start) {
      FreeResources();
