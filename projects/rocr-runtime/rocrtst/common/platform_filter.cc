...
bool TestFilterManager::initialize(const std::string& configPath) {
  ...
  // Start with the provided default path
...
void TestFilterManager::parseActiveGroups() {
  ...
  char* groupsEnv = getenv("ROCRTST_TEST_GROUPS");
  if (groupsEnv == nullptr) {
    return;  // Empty means all groups
  }

  std::string groupsStr(groupsEnv);
...
bool TestFilterManager::shouldRunTest(const std::string& testName,
  ...
  // 4. Check if ANY of test's groups are allowed on this platform
  if (!testGroups.empty()) {
