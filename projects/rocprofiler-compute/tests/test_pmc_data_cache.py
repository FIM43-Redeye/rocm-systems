...
def _make_pmc_multiindex_df() -> pd.DataFrame:
    """Build a MultiIndex DataFrame fixture with a single 'pmc_perf' table."""
    sub_df = pd.DataFrame({
...
def test_pmc_data_cache_dataframe_input() -> None:
    """MultiIndex DataFrame input yields a nested PmcDataCache with correct series."""
...
@pytest.mark.parametrize(
    "invalid_input",
    [None, [1, 2], 42, "not-a-table", {}, {"pmc_perf": pd.DataFrame()}],
)
def test_pmc_data_cache_rejects_invalid_input(invalid_input: object) -> None:
    """Construction rejects unsupported input types (including dict) with TypeError."""
...
def test_collect_debug_column_data_matches_bracketed_counter() -> None:
    ...
    cache = PmcDataCache(
        pd.concat(
            {"pmc_perf": pd.DataFrame({"TCC_HIT[0]": [10, 20, 30]})},
            axis=1,
        )
    )
