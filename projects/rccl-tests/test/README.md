...
pytest -k "parallel_init"      # -p 1 threaded init
...
pytest -k "root_rotate"        # -r all root rotation
...
pytest -k "lifecycle_single"   # lifecycle single-process only
pytest -k "lifecycle_mpi"      # lifecycle MPI only
```
...
pytest test_regression.py -k "gpus1"   # single-GPU baseline only
...
powers-of-2 up to `NGPUS`, always including `NGPUS` itself:

| Machine | NGPUS | GPU grid |
|---------|-------|----------|
| 8-GPU node | 8 | `[1, 2, 4, 8]` |
| 6-GPU node | 6 | `[1, 2, 4, 6]` |
| 4-GPU node | 4 | `[1, 2, 4]` |
| 2-GPU dev box | 2 | `[1, 2]` |
...
`--reruns=1 --reruns-delay=0.25` is enabled by default in `pytest.ini`.
Each failed test item is retried once before being reported as a failure.
Because tests are independent parametrized items (not subtests), retries
are scoped to the exact failing combination — not the entire test file.
...
| `--reruns=N` | 1 | Retry failed tests N times (set in `pytest.ini`) |
