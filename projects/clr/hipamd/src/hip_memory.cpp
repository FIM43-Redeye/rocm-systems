...
hipError_t ihipMemcpyBatch(void** dsts, void** srcs, size_t* sizes, size_t count,
  ...
  if (!bufferCopyIndices.empty()) {
    ...
    for (size_t idx : bufferCopyIndices) {
      amd::Memory* srcMemory = srcMemories[idx];
      amd::Memory* dstMemory = dstMemories[idx];
      ...
      copyOps.emplace_back(srcMemory, dstMemory, srcOffsets[idx], dstOffsets[idx], sizes[idx],
                           metadata);
    }
