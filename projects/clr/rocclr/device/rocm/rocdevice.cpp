...
bool Device::create() {
  ...
  // Try to get LUID from HSA agent properties
  hsa_status_t luid_status = Hsa::agent_get_info(
      bkendDevice_,
      static_cast<hsa_agent_info_t>(HSA_AMD_AGENT_INFO_DRIVER_UID),
      &deviceLuid_);

  if (luid_status == HSA_STATUS_SUCCESS) {
    luidValid_ = true;

    // Also get GPU index for multi-GPU chains (LDA configurations)
    uint32_t nodeId = 0;
    if (HSA_STATUS_SUCCESS ==
        Hsa::agent_get_info(bkendDevice_,
                           static_cast<hsa_agent_info_t>(HSA_AMD_AGENT_INFO_NODE_ID),
                           &nodeId)) {
      gpuIndex_ = nodeId;
    } else {
      gpuIndex_ = 0;  // Default to primary GPU if node ID unavailable
    }
  } else {
    luidValid_ = false;
    gpuIndex_ = 0;
...
#ifdef _WIN32
  // Handle D3D9 device binding
  if (flags & amd::Context::Flags::D3D9DeviceKhr) {
    void* d3d9Device = gfxDevice[amd::Context::DeviceFlagIdx::D3D9DeviceKhrIdx];
    if (!D3D9Interop::associateD3D9Device(this, static_cast<IDirect3DDevice9Ex*>(d3d9Device))) {
      LogError("Failed D3D9Interop::associateD3D9Device()");
      success = false;
    }
  }
...
#ifdef _WIN32
  // Handle D3D9 device cleanup
  if (flags & amd::Context::Flags::D3D9DeviceKhr) {
    D3D9Interop::dissociateD3D9Device(this);
  }
